...
Private Declare Function WriteProfileString Lib "kernel32" Alias "WriteProfileStringA" (ByVal lpszSection As String, _
...
Public Function DialogFont(ByVal lngHWnd As Long, _
                           c As Control) As Boolean
...
'APPROVED(Y ) c [Remove the space after the 'Y' in brackets and next run of Code Fixer will create the With Structure for you.
...
Public Function DialogPrint(ByVal lngHWnd As Long, _
                            ByVal bPages As Boolean, _
                            ByVal Flags As Long) As PRINTPROPS
